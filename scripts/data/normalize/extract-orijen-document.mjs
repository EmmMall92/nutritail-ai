...
const defaultDocumentPath = "C:/Users/NIOstb/Desktop/photo_foods_nutritail/ORIJEN.docx";

const paths = {
  template: "data/templates/nutritail-food-v2-template.csv",
  output: "data/imports/orijen_document_extract_v2.csv",
  review: "data/review/orijen_document_extract_review.csv",
  report: "reports/orijen_document_extract.md",
};
...
function formulaNameFromHeading(heading) {
  const withoutPrefix = cleanText(heading)
    .replace(/^\d+\)\s*/u, "")
    .replace(/^ORIJEN\s+/iu, "")
    .replace(/\b\d+(?:[.,]\d+)?\s*(?:GR|G|KG|KGS|OGR)\b/giu, "")
    .replace(/\s*\/\s*/g, " ")
    .replace(/\s+/g, " ")
    .trim();

  if (/^CAT\s*&\s*KITTEN$/iu.test(withoutPrefix)) return "Cat & Kitten";

  return titleCase(withoutPrefix.replace(/\bCAT\b/giu, ""));
}
...
function commercialTagsFor(formulaName, ingredients) {
  ...
  if (text.includes("γαλοπούλ")) tags.push("turkey");
  if (text.includes("πάπια")) tags.push("duck");
  if (text.includes("αρν")) tags.push("lamb");
  if (text.includes("βοδιν") || text.includes("βίσω") || text.includes("αγριόχοιρ") || text.includes("χοιριν")) {
    tags.push("red_meat");
  }
  if (
    text.includes("ρέγκα") ||
    text.includes("ψάρ") ||
    text.includes("μπακαλιάρ") ||
    text.includes("σαρδέλα") ||
    text.includes("σκουμπρί") ||
    text.includes("πέστροφα") ||
    text.includes("καλκάνι")
  ) {
    tags.push("fish");
  }
...
function rowFromBlock(block, sourcePath) {
  ...
  const formulaKey = `orijen-${slugify(block.formulaName)}-cat-dry-gr-document`;
  const epaDha = combinedEpaDhaNote(block.analysisText);

  return {
    brand: "ORIJEN",
    formula_name: block.formulaName,
    display_name: `ORIJEN ${block.formulaName}`,
    species: "cat",
    format: "dry",
    ...
    fat_sources: detectTerms(ingredients, ["λίπος", "λάδι", "έλαιο", "ηλιέλαιο"]).join(";"),
    fiber_sources: detectTerms(ingredients, ["ίνες", "κολοκύθα", "φύκια", "δαυκία", "σπανάκι", "άλφα-άλφα"]).join(";"),
    additives_text: "",
    feeding_guide_text: "",
    kcal_per_100g: "",
    kcal_per_kg: "",
    ...
    source_notes: [
      ...
      "kcal_required=true",
      epaDha,
      "Auto-extracted from ORIJEN.docx; verify against official ORIJEN source or label before import.",
    ]
      .filter(Boolean)
      .join("; "),
...
function isHeading(value) {
  ...
  if (/^ORIJEN$/iu.test(text)) return false;
  return /^(?:\d+\)\s*)?ORIJEN\b/iu.test(text);
}
...
function extractBlocks(paragraphs) {
  ...
  for (let index = 0; index < paragraphs.length; index += 1) {
    ...
    const nearby = paragraphs.slice(index + 1, index + 8);
    const ingredientParagraph = nearby.find((item) => /^Σύνθεση\s*:/iu.test(item));
    const analysisParagraph = nearby.find((item) => /^Αναλυτικά συστατικά\s*:/iu.test(item));
...
function reviewRow(row) {
  return {
    ...
    recommended_action:
      "Add official ORIJEN URL or label evidence, backfill kcal/ME, then preview in Food V2 before commit.",
    notes:
...
function renderReport(rows, duplicates, sourcePath) {
  ...
  return `# ORIJEN Document Extract
...
async function main() {
  ...
  console.log(`ORIJEN document rows: ${rows.length}`);
