...
const advisoryPassingSuites = parsedAdvisorySuites.filter((suite) => suite.status === "PASS");
const advisorySkippedSuites = parsedAdvisorySuites.filter((suite) => suite.status === "SKIPPED");
const fullOpenAiProofStatus =
  parsedAdvisorySuites.length > 0 && advisoryPassingSuites.length === parsedAdvisorySuites.length
    ? "PASS"
    : advisorySkippedSuites.length > 0
      ? "PENDING"
      : "REVIEW";
const customerReadyCoreStatus = failingSuites.length === 0 ? "PASS" : "REVIEW";
...
const lines = [
  ...
  "- Score type: automated live readiness, not subjective customer product completeness",
  "- Customer product progress: tracked separately through live UX findings, recommendation accuracy reviews, and the next-task list",
  ...
  "- Automated live readiness and customer product progress are different: this dashboard proves deploy/readiness evidence, while customer product progress still depends on live UX findings and real-flow polish.",
